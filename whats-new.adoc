...
数据库工作负载工厂改进了其在Amazon FSx for NetApp ONTAP存储上运行的实例的注册流程。您现在可以批量选择实例进行注册。一旦实例注册，您就可以在工作负载工厂控制台内创建和管理数据库资源。
...
数据库实例的良好架构状态现在包括对 Microsoft 多路径 I/O (MPIO) 超时设置的分析和修复。将 MPIO 超时设置为 60 秒可确保 FSx for ONTAP存储在故障转移期间的连接性和稳定性。如果未相应地设置 MPIO 设置，工作负载工厂将提供修复程序以将 MPIO 超时值设置为 60 秒。
